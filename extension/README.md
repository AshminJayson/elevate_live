...
so viewers follow your typing live in the code pane — no save required. (The
...
sends `{type:"file", path, content}` over a persistent `/ws/host` WebSocket —
...
The host **token** is entered via a prompt the first time you stream and kept
...
|---|---|---|
| `bitforge.serverUrl` | `ws://127.0.0.1:8000` | Hub base; connects to `<serverUrl>/ws/host`. |
| `bitforge.sourceDir` | _(workspace folder, or `BITFORGE_SOURCE_DIR` from a workspace `.env`)_ | Source root; only files under it stream. |
...
your source workspace (with `.env` containing `BITFORGE_TOKEN`) while the BitForge
